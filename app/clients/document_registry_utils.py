...
def build_applicability_profile(values: dict[str, Any]) -> dict[str, Any]:
    """生成版本适用范围；同一适用范围内只能有一个生效revision。"""
    ...
    applicability_key = "default" if not normalized.replace("\0", "") else f"scope-{hashlib.sha256(normalized.encode('utf-8')).hexdigest()[:24]}"
...
class DocumentRegistry(ABC):
    ...
    def managed_revision_access(
        ...
        """批量判断治理版本是否允许参与查询；调用方只传治理标记为真的Chunk。"""
        raise NotImplementedError
