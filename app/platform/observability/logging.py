...
_LOGGING_MODULE_PATH = Path(__file__).resolve()

def fix_log_position(record):
    """遍历调用栈，跳过loguru内部帧+工具类自身帧，提取业务代码实际调用位置"""
    for frame in inspect.stack():
        frame_path = Path(frame.filename).resolve()
        if frame_path == _LOGGING_MODULE_PATH or "_logger.py" in frame_path.name or frame.function == "_log":
            continue
        # 更新日志字段为业务代码实际位置
        record.update(
...
if __name__ == '__main__':
    logger.info("【测试】日志模块内部调用（仅测试，业务模块调用会显示正确文件名）")
