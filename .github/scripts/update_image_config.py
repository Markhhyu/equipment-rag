...
# 本地首次验证建议使用 smart，既保留图片检索能力，也避免对已有清晰图注重复调用视觉模型。
...
# Worker 领取图片后的租约时间，单位秒；应大于正常单张图片处理时间，进程崩溃后过期任务可重新领取。
...
# 当前进程内所有图片 Worker 合计每分钟最多发起的视觉请求数，应按模型服务限流配置调整。
...
# 保存图片前后文的最大字符数；调大可增加语义，但也会增加 MongoDB 记录和提示词长度。
IMAGE_CONTEXT_CHARS=240

# 查询阶段是否允许根据用户的图片、界面、按钮位置等问题按需调用视觉模型。
QUERY_IMAGE_VISION_ENABLED=true
# 每次查询最多选择的相关图片数；调大可能提升覆盖，但会增加延迟、Token 和图片输入成本。
QUERY_IMAGE_TOP_K=3
# 查询阶段视觉模型请求超时，单位秒；超时后会降级使用已缓存图片说明，不中断文本回答。
QUERY_IMAGE_VISION_TIMEOUT_SECONDS=45

"""

COMPOSE_MARKER = "  # Embedding 和 Reranker 模型。\n"
COMPOSE_BLOCK = """  # 图片资产化、异步视觉增强和查询阶段按需视觉推理；默认值与代码配置保持一致。
...
KEYS = [
    ...
    "IMAGE_ENRICHMENT_ASYNC",
    "IMAGE_ENRICHMENT_WORKERS",
    "IMAGE_ENRICHMENT_POLL_SECONDS",
    "IMAGE_ENRICHMENT_LEASE_SECONDS",
    "IMAGE_CAPTION_MAX_PER_DOCUMENT",
    "IMAGE_CAPTION_TIMEOUT_SECONDS",
    "IMAGE_CAPTION_MAX_RETRIES",
    "IMAGE_CAPTION_REQUESTS_PER_MINUTE",
    "IMAGE_MIN_BYTES",
    "IMAGE_MAX_BYTES",
    "IMAGE_STRONG_CAPTION_MIN_CHARS",
    "IMAGE_CONTEXT_CHARS",
    "QUERY_IMAGE_VISION_ENABLED",
    "QUERY_IMAGE_TOP_K",
    "QUERY_IMAGE_VISION_TIMEOUT_SECONDS",
]
