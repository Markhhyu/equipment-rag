...
运营看板推荐从统一入口 `http://localhost:8080/apps` 打开；兼容地址为
`http://localhost:8001/analytics.html`。其中“技术成功”只表示Agent正常完成，
...
新导入版本会把质量报告、发布门禁结论和均匀抽取的切片样本保存到版本记录。管理员可在知识库治理页审核；`blocked`版本保留解析文件和向量用于排查，但自动发布、手动发布和回滚都会被拒绝。升级前没有质量报告的历史版本保持兼容，不会被门禁锁定。

门禁阈值由`KNOWLEDGE_QUALITY_*`环境变量控制。综合分、合理长度切片比例、重复率、设备名称覆盖率可以按真实手册基线调整；正文为空、向量生成不完整或Milvus入库不完整始终作为明确的质量失败展示，避免平均分掩盖关键故障。
...
查询图会先生成确定性检索计划：精确参数和故障码只使用本地混合检索，模糊故障排查按需增加HyDE，只有用户明确要求联网、官网或最新资料时才调用Web Search。高风险操作不会使用外部网页作为检索分支。Langfuse中的`node_query_router` Span会记录问题类型和实际启用的分支，不记录额外正文。

Reranker 默认至少保留两条证据。真实手册中的状态说明、操作前提和精确规格经常位于相邻切片；只保留 Top1 虽然上下文更短，但可能让模型在证据缺口处补造参数。可通过 `RAG_RERANK_MIN_TOPK` 调整，但不建议低于 `2`。
