...
| Vue 业务页面 | Vue 3 + TypeScript 多入口页面，共享身份守卫、角色菜单、API Key 设置和模块级访问控制 |
...
| 服务 | 默认地址 | 用途 |
|---|---|---|
| 统一前端入口 | <http://127.0.0.1:8080/apps> | 推荐入口；按当前角色展示并访问全部业务页面 |
| 导入页面（兼容） | <http://127.0.0.1:8000/import.html> | 上传 PDF/Markdown 并查看导入进度 |
| 知识库治理（兼容） | <http://127.0.0.1:8000/knowledge.html> | 文档版本、发布、回滚、停用和操作审计 |
| 聊天页面（兼容） | <http://127.0.0.1:8001/chat.html> | 多轮问答、结构化来源、图片展示和反馈 |
| 问答运营看板（兼容） | <http://127.0.0.1:8001/analytics.html> | 每日问答、技术成功率、解决结果和待关注问题 |
...
1. 从统一入口 <http://127.0.0.1:8080/apps> 打开“知识库治理”；
...
统一入口中的“资料导入”是兼容原有习惯的快速导入入口，导入完成后会自动发布。它适合本地试验；企业生产环境优先使用治理入口。
...
从统一入口 <http://127.0.0.1:8080/apps> 打开“智能问答”，建议在问题里包含明确型号：
...
| `GET/PUT/DELETE` | `/workflow/connectors/feishu` | 读取、保存或清除当前租户的飞书审批配置；密钥不回显 |
| `POST` | `/workflow/connectors/feishu/test` | 验证飞书应用凭证、权限和审批定义 |
...
管理员可在人工处理中心右上角打开“飞书设置”，按租户维护应用凭证、当前默认审批定义和发起人。
`APP_SECRET` 由后端加密保存且不会回显；页面配置存在时优先于环境变量，保存后无需重启服务。
